<script lang="ts">
	...
	function fitText(
		...
		while (
			...
			ctx.measureText(`${fittedChars.join("")}${ellipsis}`).width >
				maxWidth
		) {
			fittedChars.pop();
		}
	...
	async function generatePoster() {
		...
		try {
			...
			// Parse Date
			let dateObj: { day: string; month: string; year: string } | null =
				null;
			try {
